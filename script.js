...
let crntTime = () => {
    currentTime.textContent = `${new Date().getHours()} : ${new Date().getMinutes()} : ${Number(new Date().getSeconds()) < 10 ? '0' + Number(new Date().getSeconds()) : Number(new Date().getSeconds())}`
}

setInterval(crntTime, 100)
                ...
                .then((valu) => {
                    ...
                    for (let item = 0; item < info.length; item++) {
                        ...
                        cardConteiner.addEventListener('click', () => {
                            cityInput.value = info[item].name
                            getWeather()
                        })
                    }
                })
    } catch (e) {
        span.textContent = "Oops! City not found. Try again."
